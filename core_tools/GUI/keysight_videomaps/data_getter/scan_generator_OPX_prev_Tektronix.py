...
### Originally copied from scan_generator_Tektronix.py (ma)
...
def construct_1D_scan_fast(gate, swing, n_pt, t_step, biasT_corr, pulse_lib, digitizer, channels,
                           dig_samplerate=60e6, dig_vmax=None, iq_mode=None, acquisition_delay_ns=None,
    ...
    return _digitzer_scan_parameter_OPX(digitizer, my_seq, pulse_lib, t_step, acquisition_delay_ns, n_lines, line_delay_pts,
...
def construct_2D_scan_fast(gate1, swing1, n_pt1, gate2, swing2, n_pt2, t_step, biasT_corr, pulse_lib,
                           digitizer, channels, dig_samplerate=60e6, dig_vmax=None, iq_mode=None,
    ...
    # generate the sequence and upload it.
    my_seq = pulse_lib.mk_sequence([seg])
    #my_seq.set_hw_schedule(TektronixSchedule(pulse_lib))  #not a tektronix
    my_seq.configure_digitizer = False
    my_seq.n_rep = 1
    # my_seq.sample_rate = sample_rate   # sample_rate is digitizer function that isnt avaialble yet

    logger.info(f'Seq upload')
    #my_seq.upload()    # upload is digitizer function that isnt avaialble yet

    n_lines = n_pt2
    scan_params = _digitzer_scan_parameter_OPX(digitizer, my_seq, pulse_lib, t_step, acquisition_delay_ns, n_lines, line_delay_pts,
...
class _digitzer_scan_parameter_OPX(MultiParameter):
    """
    ...
    def __init__(self, digitizer, my_seq, pulse_lib, t_measure, acquisition_delay_ns,
        ...
        # digitizer.sample_rate(sample_rate)          # no sample_rate yet (ma)
        # self.sample_rate = digitizer.sample_rate()  # no sample_rate yet (ma)
        self.sample_rate = sample_rate   # do this instead! (ma)

        if self.n_ch not in [1,2,4]:
            raise Exception('Number of enabled channels on M4i must be 1, 2 or 4.')
        # digitizer.enable_channels(sum([2**ch for ch in self.channels])) # no enable_channels yet (ma)
        ...
        # self.dig.trigger_or_mask(pyspcm.SPC_TMASK_EXT0)  # no digitizer yet (ma)
        # self.dig.setup_multi_recording(self.seg_size, n_triggers=1) # no digitizer yet (ma)
        ...
        # Run the OPX startup script which configures the OPX 
        #import opx_functions
        import core_tools.GUI.keysight_videomaps.data_getter.opx_functions as opx_functions
        self.opx = opx_functions.startup_2d()

        # Create the qcodes mulitparameters for the X and Y axes (self.names),  with an array of setpoints (setpoint) 
        super().__init__(name=digitizer.name, names=self.names,
                        ...
                        docstring='Scan parameter for OPX')
        
        # Loads the QUA code and then starts the qcodes measurement dataserver process in an infinite loop
        self.opx.run_2d(self.opx)
    ...
    def get_raw(self):
        ...
        raw_data = self.opx.get_data()
        
        # Previous Tektronix code (for reference)
        # # play sequence
        ...
        point_data = np.zeros((len(self.channels), self.n_points), dtype=raw_data.dtype)
