...
class SQL_database_manager():
    ...
    def remote_connection(self):
        cls = SQL_database_manager
        if cls._remote_connection is None or getattr(cls._remote_connection, "closed", False):
            cls._connect_remote()
    ...
    @classmethod
    def _connect(cls):
        if cls._connection is not None and not getattr(cls._connection, "closed", False):
            cls._connection.close()
    ...
    def _connect_remote(cls):
        if cls._remote_connection is not None and not getattr(cls._remote_connection, "closed", False):
            cls._remote_connection.close()
