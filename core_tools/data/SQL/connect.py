...
def set_up_local_and_remote_storage(
    ...
    Args:
        ...
        scope (str|None) : SQDL scope name, can be None for setups that do not use SQDL.
    """
    SQL_conn_info_local('localhost', 5432, user_local, passwd_local, dbname_local, local_readonly, local_is_sqlite)
    SQL_conn_info_remote(host, port, user_remote, passwd_remote, dbname_remote, remote_readonly, remote_is_sqlite)
    sample_info(project, set_up, sample, scope)
