...
from core_tools.data.SQL.model.version import DatabaseVersion, get_database_version
...
class measurement_overview_queries:
    ...
    def new_measurement(conn, exp_name, start_time):
        ...
        """
        if (
            not is_valid_info(sample_info.project)
        ...
        if get_database_version(conn, False) >= DatabaseVersion("1.1.0"):
            ...
            var_values = (
                uuid,
                str(sample_info.set_up),
                scope_value,
                str(sample_info.project),
                str(sample_info.sample),
                username,
                exp_name,
                "New measurement_parameters",
                "",
                psycopg2.sql.SQL("TO_TIMESTAMP({})").format(
                    psycopg2.sql.Literal(start_time)
                ),
            )
        else:
            var_names = (
    ...
    def update_measurement(
        ...
        if stop_time is not None:
            var_pairs.append(
                (
                    "stop_time",
                    psycopg2.sql.SQL("TO_TIMESTAMP({})").format(
                        psycopg2.sql.Literal(stop_time)
                    ),
                )
            )
        if metadata is not None:
            var_pairs.append(
                ("metadata", psycopg2.Binary(str(json.dumps(metadata)).encode("ascii")))
            )
        if snapshot is not None:
            var_pairs.append(
                ("snapshot", psycopg2.Binary(str(json.dumps(snapshot)).encode("ascii")))
            )
        ...
        update_table(
            conn, "global_measurement_overview", var_names, var_values, condition
        )
    ...
    def is_completed(conn, uuid):
        completed = execute_query(
            conn,
            f"SELECT completed FROM global_measurement_overview where uuid = {uuid};",
        )
...
class data_table_queries:
    ...
    def generate_table(conn, table_name):
        ...
        statement += "oid INT, "
        statement += (
            "synchronized BOOL DEFAULT False,"  # Note [SdS]: Column is not used
        )
    ...
    def insert_measurement_spec_in_meas_table(conn, table_name, data_item):
        ...
        var_values = (
            ...
            data_item.nth_set,
            data_item.nth_dim,
            data_item.param_id_m_param,
            data_item.setpoint,
            data_item.setpoint_local,
            data_item.name_gobal,
            data_item.name,
            data_item.label,
            data_item.unit,
            psycopg2.extras.Json(data_item.dependency),
            psycopg2.extras.Json(data_item.shape),
            0,
            data_item.size,
            data_item.oid,
        )
    ...
    def insert_measurement_params(conn, exp_uuid, data_items):
        ...
        for index, item in enumerate(data_items):
            var_values = (
                ...
                item.label,
                item.unit,
                psycopg2.extras.Json(item.dependency),
                psycopg2.extras.Json(item.shape),
                0,
                item.size,
                item.oid,
            )
