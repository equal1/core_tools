...
class sync_mgr_queries:
    ...
    def sync_raw_data(sync_agent, uuid, to_local=False):
        ...
        if to_local:
            raw_data_table_name, sync_location = select_elements_in_table(
                conn_src,
                'global_measurement_overview',
    ...
    def convert_SQL_raw_table_entry_to_python(content):
        ...
        content['start_time'] = psycopg2.sql.SQL("TO_TIMESTAMP({})").format(
            psycopg2.sql.Literal(content['start_time'].timestamp()))
        ...
        if content['stop_time'] is not None:
            content['stop_time'] = psycopg2.sql.SQL("TO_TIMESTAMP({})").format(
                psycopg2.sql.Literal(content['stop_time'].timestamp()))
