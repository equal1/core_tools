...
class data_set_raw:
    ...
    measurement_parameters: list = field(default_factory=list)
    measurement_parameters_raw: list = field(default_factory=list)
    ...
    def generate_keywords(self):
        ...
        for m_param in self.measurement_parameters_raw:
            ...
            if m_param.setpoint or m_param.setpoint_local:
                set_param += [label]
            else:
                get_param += [label]
...
class m_param_raw:
    ...
    def __copy__(self):
        ...
        return m_param_raw(
            copy.copy(self.param_id), copy.copy(self.nth_set),
            copy.copy(self.nth_dim), copy.copy(self.param_id_m_param),
            copy.copy(self.setpoint), copy.copy(self.setpoint_local),
            copy.copy(self.name_gobal), copy.copy(self.name), copy.copy(self.label),
            copy.copy(self.unit), copy.copy(self.dependency),
            copy.copy(self.shape), copy.copy(self.size), copy.copy(self.oid),
            data_buffer
        )
