...
class virtual_gates_mgr:
    ...
    def add(
        ...
        name,
        gates: list[str],
        virtual_gates: list[str] | None = None,
        matrix=None,
        normalization=False,
    ):
        ...
        virtual_gate_matrix = load_virtual_gate(
            name, gates, virtual_gates, matrix, normalization=normalization
        )
...
class awg2dac_ratios_mgr:
    ...
    def add(self, gates):
        ...
        try:
            ...
            # Database might not be configured; fall back to defaults
            pass

        for gate in gates:
            self._ratios[gate] = ratios_db.get(gate, 1)
    ...
    def __setitem__(self, gate, value):
        ...
        if gate not in self._ratios.keys():
            raise ValueError(
                "Gate {} not defined in AWG2dac ratios. Please add first.".format(gate)
            )
        ...
        try:
            ...
            # Allow operation without a database connection
            pass
...
class hardware(qc.Instrument):
    ...
    def get_idn(self):
        return dict(
            vendor="CoreTools", model="hardware", serial="", firmware=ct_version
        )
    ...
    def snapshot_base(self, update=False, params_to_skip_update=None):
        ...
        return {
            "awg2dac_ratios": self.awg2dac_ratios._ratios,
