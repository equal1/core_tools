...
def load_virtual_gate(
    name, real_gates, virtual_gates=None, matrix=None, normalization=False
):
    ...
    try:
        ...
        if virtual_gate_queries.check_var_in_table_exist(conn, name):
            real_gate_db, virtual_gate_db, matrix_db = (
                virtual_gate_queries.get_virtual_gate_matrix(conn, name)
            )
...
def save_virtual_gate(vg_matrix):
    ...
    if virtual_gate_queries.check_var_in_table_exist(conn, vg_matrix.name):
        ...
        real_gate_db, virtual_gate_db, matrix_db = (
            virtual_gate_queries.get_virtual_gate_matrix(conn, vg_matrix.name)
        )
        ...
        virtual_gate_queries.set_virtual_gate_matrix(
            conn, vg_matrix.name, all_real_gates, all_virtual_gates, matrix
        )
    else:
        virtual_gate_queries.set_virtual_gate_matrix(
            conn,
            vg_matrix.name,
            vg_matrix.real_gate_names,
            vg_matrix.virtual_gate_names,
