...
from core_tools.sweeps.sweep_utility import (
    SequenceStartAction, get_pulselib_sweeps, sweep_info
)
...
class scan_generic(metaclass=job_meta):
    def __init__(self, *args, name="", reset_param=False, silent=False):
        ...
        Args:
            ...
            reset_param (bool) : reset the setpoint parametes to their original
                value after the meaurement
            silent (bool) : If True do not print dataset id and progress bar
        """

        warnings.warn(
            (
                "'scan_generic' class will be removed in a later version of "
                "core-tools, to be replaced with the 'Scan' class from core_tools."
                "sweeps.scans: see examples for more info."
            ),
            DeprecationWarning,
        )
        ...
        if name == "":
            if len(self.set_vars) == 0:
                ...
                print(
                    "WARNING: no name specified with scan! Please specify a name."
                )
