...
class var_sql_queries:
    ...
    def init_table(conn):
        ...
        statement = (
            "CREATE TABLE if not EXISTS {} (id SERIAL, insert_time TIMESTAMP );".format(
    ...
    def add_variable(conn, name, unit, category, step, value=0):
        ...
        if len(res) == 0:
            insert_row_in_table(
                conn,
                var_sql_queries.gen_table_overview_name(),
                ("name", "unit", "category", "step"),
                (name, unit, category, step),
            )
            alter_table(
                conn, var_sql_queries.gen_table_content_name(), (name,), ("FLOAT8",)
            )
    ...
    def get_all_specs(conn):
        return select_elements_in_table(
            conn,
            var_sql_queries.gen_table_overview_name(),
            ("*",),
    ...
    def get_history(conn, variable_name):
        ...
        for name in [sql.Identifier("insert_time"), sql.Identifier(variable_name)]:
            ...
            query += sql.SQL("WHERE {0} IS NOT NULL ").format(
                sql.Identifier(variable_name)
            )
            query += sql.SQL("ORDER BY {0} {1} ").format(
                sql.Identifier("id"), sql.SQL("ASC")
            )
    ...
    def get_values_at(conn, time):
        ...
        """
        query = (
            f"SELECT max(insert_time) FROM {var_sql_queries.gen_table_content_name()} "
        )
    ...
    def remove_variable(conn, variable_name):
        statement_1 = sql.SQL("DELETE FROM {} WHERE {} = {} returning name").format(
            sql.SQL(var_sql_queries.gen_table_overview_name()),
            sql.Identifier("name"),
            sql.Literal(variable_name),
        )
        statement_2 = sql.SQL("ALTER TABLE {} DROP COLUMN IF EXISTS {}").format(
            sql.SQL(var_sql_queries.gen_table_content_name()),
            sql.Identifier(variable_name),
        )
        ...
        if len(res) == 0:
            print(
                "Nothing to remove. {} is not present in the database?".format(
                    variable_name
                )
            )
